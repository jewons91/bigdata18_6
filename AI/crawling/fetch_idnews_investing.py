...
from investingCrawling.insert_idnews_to_db import insert_idnews_to_db
...
def get_old_idnews_from_db(GINDEX_CODE, config_file_path): 
    conn.connect_to_database(file_path=config_file_path)
...
def insert_id_db(search_word, url_start, config_file_path): 
    today = datetime.today().strftime('%Y-%m-%d')
    GINDEX_CODE = search_GINDEX_CODE(search_word, config_file_path)
    if GINDEX_CODE:
        db_df = get_old_idnews_from_db(GINDEX_CODE, config_file_path)
    logging.info('###############################################################')
    logging.info(db_df)
    logging.info('###############################################################')
    ...
    for idx, row in news_df.iterrows():
        ...
        row_data = pd.DataFrame({ 
            'USIDXN_CODE' : ['USN'+ time_str.replace('-', '').replace(':', '').replace(' ', '') + new_index_str if time_str else 'USN00000000000000'], 
...
def consim_id(data, db_df, config_file_path): 
    ...
    if combined_df.empty:
        ...
        if not news_json_df.empty:
            ...
            logging.info('#####################################################')
            logging.info('#####################################################')
            insert_idnews_to_db(news_json, config_file_path)  
        else:
            logging.info('기사 없어서 json변환 건너뜀')



def process_and_compare_idnews(config_file_path):
    source = [
    ...
    for idx, row in insert.iterrows():
        ...
        combined_df = consim_id(data, db_df, config_file_path) 
            
# process_and_compare_idnews()  # insert_db 호출
