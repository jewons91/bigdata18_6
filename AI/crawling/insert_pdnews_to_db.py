...
def insert_pdnews_to_db(news_json,config_file_path):
    try:
        conn.connect_to_database(file_path=config_file_path)
        data = json.loads(news_json)
        
        insert_query = """
        INSERT INTO TB_USPRODUCTNEWS (USPDTN_CODE, PDT_CODE, INVEST_CODE, USNEWS_TITLE, USNEWS_CONTENT, USNEWS_DATE, USNEWS_PRESS, USNEWS_URL)
